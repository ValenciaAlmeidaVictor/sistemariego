                        ...
                        <div class="card-body px-0 pb-2">
                            <div class="table-responsive p-0">
                                <table class="table align-items-center mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($plantas as $plata)
                                    <tr>
                                        <td>{{$planta->id}}</td>
                                        <td>{{$planta->nombre}}</td>
                                        <td>{{$planta->descripcion}}</td>
                                        <td>
                                            <form action="{{route('posts.destroy',$planta->id)}}" method="post">
                                                <a href="{{route('posts.edit',$planta->id)}}" class="btn btn-warning btn"><i class="fa-regular fa-pen-to-square"></i></a>
                                                @csrf
                                                @method('DELETE')
